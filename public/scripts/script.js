...
var looksDB = firebase.database().ref('looks');
...
makeupApp.loadLooks = function () {
	...
	makeupApp.looks.forEach(function (look) {
		...
		templateItem.find('.like-button').on('click', function () {
			//selects template item
			//find the like button, on click, function runs
			if ($("#likes-cell-" + look.id + " .like-button").hasClass('liked')) {
				looksDB.update(_defineProperty({}, "look" + look.id, Object.assign({}, look, {
...
makeupApp.productGallerySetup = function (look) {
	...
	var _loop = function _loop() {
		var productInfo = makeupApp.products[look.products[productID]];
		productTemplateItem = $(productTemplate);
		...
		carouselTemplate = $('#product-carousel').html();
		...
		productTemplateItem.find('.add-to-total').on('click', function () {
			...
			var totalValue = totalArr.reduce(function (acc, value) {
				return acc + value;
			}, 0);
...
makeupApp.looksGallerySetup = function () {
	...
	var looksGallery = $('.looks-gallery').isotope({
		...
		layoutMode: 'fitRows',
		getSortData: {
			popular: function popular(itemElem) {
				// function
			...
			oldest: function oldest(itemElem) {
				// function
				var orderAdded = $(itemElem).attr('data-order-added');
				return parseInt(orderAdded);
			}
	...
	filterButtons.on('click', 'button', function () {
		...
		if (filterValue !== '*') {
			// if a filter other than All is clicked
